...
class ExpenseController extends Controller
{
    ...
    public function addExpensePayment(Request $request)
    {
        ...
        $paymentReceived->payment_type = @$request->get('method');
    ...
    public function store(Request $request)
    {
        ...
        return redirect()->route('expense.index')->with('success', 'تم إنشاء المصروفات بنجاح');
    }
    ...
    public function editExpense(Request $request)
    {
        ...
        Session::flash('success', "تم تحديث المصاريف بنجاح.");
        ...
        if (!empty($terms)) {
            foreach ($terms as $Key => $term) {
                ...
                {$token}
                            <input name='_method' type='hidden' value='DELETE'>
                            <button class='dropdown-item text-center' type='submit'  style='background: transparent;
    border: none;'><i class='fa fa-trash fa-1x'></i> حذف</button>
                          </form>";

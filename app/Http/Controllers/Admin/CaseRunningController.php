    ...
    public function client_case_list(Request $request)
    {
        ...
        if (!empty($cases)) {
            foreach ($cases as $key => $case) {
                ...
                $nestedData['court'] = '<p class="currenttittle">محكمة :<b> ' . $case->court_name . '</b></p>
                                        <p class="currenttittle">رقم المحكمة:<b> ' . $case->court_no . '</b></p>
                                        <p class="currenttittle">قاضي :<b> ' . $case->judge_name . '</b></p>';
        ...
        foreach ($customcollections as $key => $case) {
            ...
            $row['name'] = '<div style="font-size:15px;"  class="clinthead text-primary">' . $priorityModal . ''
                . '&nbsp;<a class="title text-primary"  href="' . $case_list . '">' . $case->first_name . '&nbsp;' . $case->middle_name . '&nbsp;' . $case->last_name . '</a></div>
                                        <p class="clinttittle">رقم القضية :<b>' . $case->case_number . '</b></p>
                                        <p class="clinttittle">القضية: <b>' . $case->caseType . '</b></p>
                                        <p class="clinttittle"><b>يسند إلى</b></p>
                                        ' . $members;
            ...
            $row['case'] = '<p class="currenttittle">' . $first . ' <br/><b>ضد</b> <br/>' . $second . '<p>';
            ...
            if ($case->id) {
                ...
                if (isset($request->tasks) && count($request->tasks)) {
                    foreach ($request->tasks as $key => $task) {
                        ...
                        $task_item = new Task();
                        $task_item->rel_type  = 'case';
                        $task_item->rel_id = $case->id;
                        $task_item->task_subject = $task['task_subject'];
                        $task_item->start_date = $task['start_date'];
                        $task_item->end_date = $task['end_date'];
                        $task_item->save();

                        if (isset($task['assigned_to']) && count($task['assigned_to'])){
                            foreach ($task['assigned_to'] as   $member) {
                                # Arrary in assigne employee...
                                $taskMember = new TaskMember();
                                $taskMember->task_id = $task_item->id;
                                $taskMember->employee_id = $member;
                                $taskMember->save();
                            }
                        }
...
//            'position' => 'required',
            ...
            'tasks.*.assigned_to' => 'required|array',
            'tasks.*.assigned_to.*' => 'required|exists:admins,id',
            'tasks.*.task_subject' => 'required|string|max:255',
    ...
    public function edit($id)
    {
        ...
        $data['tasks'] = Task::query()->where('rel_type', '=', "case")->where('rel_id' , $id)->with('members')->get();
            ...
            if ($case->id) {
                ...
                if (isset($request->tasks) && count($request->tasks)) {
                    foreach ($request->tasks as $key => $task) {
                        ...
                        TaskMember::where('task_id', $task_item->id)->delete();
                        ...
                        $to_be_not_deleted[] = $task_item->id;
                    }

                    Task::query()->where('rel_type' , 'case')->where('rel_id' , $id)->whereNotIn('id',$to_be_not_deleted)->delete();
                }
